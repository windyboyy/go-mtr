...
// mtr的实现
func runMtr(destAddr string, options *mtr.MtrOptions) (result mtr.MtrResult, err error) {
	...
	mtrResults := make([]*mtr.MtrReturn, options.MaxHops()+1)
	...
	for snt := 0; snt < options.SntSize(); snt++ {
		for ttl := 1; ttl < options.MaxHops(); ttl++ {
			if mtrResults[ttl] == nil {
				mtrResults[ttl] = &mtr.MtrReturn{TTL: ttl, Host: "???", SuccSum: 0, Success: false, LastTime: time.Duration(0), AllTime: time.Duration(0), BestTime: time.Duration(0), WrstTime: time.Duration(0), AvgTime: time.Duration(0)}
			}
